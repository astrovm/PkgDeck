...
Run `pkd` in a terminal. Repeatable `--from apt|dnf|pacman|zypper|snap|homebrew|appimage|flatpak|cargo|npm|pnpm|bun|pip|pipx|uv|composer|gem` restricts sources and `--arch`
