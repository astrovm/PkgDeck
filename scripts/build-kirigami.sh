#!/usr/bin/env bash
set -euo pipefail
# Qt 6.8.3 must already be on PATH/CMAKE_PREFIX_PATH.
...
for project in extra-cmake-modules kirigami; do
    archive="$work/$project-6.12.0.tar.xz"
    curl --fail --location --retry 3 "https://download.kde.org/stable/frameworks/6.12/$project-6.12.0.tar.xz" -o "$archive"
    curl --fail --location --retry 3 "https://download.kde.org/stable/frameworks/6.12/$project-6.12.0.tar.xz.sha256" -o "$archive.sha256"
    (cd "$work" && sha256sum --check "$project-6.12.0.tar.xz.sha256")
    tar -xf "$archive" -C "$work"
    cmake -S "$work/$project-6.12.0" -B "$work/$project-build" -G Ninja \
